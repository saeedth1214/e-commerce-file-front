<template>
  ...
  >
    ...
    <template v-slot:top>
      <v-toolbar flat>
        <v-toolbar-title>لیست ویژگی ها</v-toolbar-title>
        ...
        <v-dialog v-model="dialogDelete" max-width="500px">
          <v-card style="background-color: #fff">
            <v-card-title class="text-h5"
              >آیا از حذف ویژگی مطمعن هستید؟</v-card-title
            >
            <v-card-actions>
              <v-spacer></v-spacer>
...
export default {
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1 ? "افزودن ویژگی" : "ویرایش ویژگی";
    },
    headers() {
      return this.$store.state.option.attribute.headers;
    },
  },
  ...
  methods: {
    ...
    async deleteItemConfirm() {
      await this.$axios
        .delete(`panel/attributes/${this.editedItem.id}`)
        .then((res) => {
          if (res.status === 204) {
            this.dialogDelete = false;
            this.showMessage("success", " .ویژگی حذف شد");
            this.page = 1;
            this.initialize();
          }
        });
