<template>
  <v-app id="inspire">
    <v-container v-if="!loading">
      <TheAdvanceFilter @filterItems="filterPlans" cardTitle="جستجوی طرح"/>
      <v-divider></v-divider>
      <v-toolbar flat>
        <v-row style="text-align:end;">
          <v-col cols="12" sm="4" offset-md="8" class="text-left">
            ...
            <NewPlan :isActive="dialog" @close="dialog=false" @initializeEmit="initialize"/>
          </v-col>
        </v-row>
      </v-toolbar>
      <v-row col="mt-n6" v-if="plans.length">
        <v-col cols="12" md="4" sm="6" lg="4" v-for="(plan,Idx) in plans" :key="`plan-${Idx}`">
              ...
              <v-row dense>
                ...
                <v-col cols="12">
                  ...
                  >
                    <span
                      class="blue--text accent-2 text-h4 font-weight-bold"
                    >{{plan.percentage ? plan.rebate : $calculateRebate(plan)}}</span>

                    <span v-if="plan.percentage" class="font-weight-bold black--text">/ درصد تخفیف</span>
                    <span v-else class="font-weight-bold black--text">/ هزار تومان تخفیف</span>
                    <v-divider></v-divider>
                  </p>
                </v-col>
                <v-col cols="12">
      ...
      <v-row dense v-else>
        <v-alert width="100%" dense type="info" class="text-center">مورد خاصی یافت نشد</v-alert>
      </v-row>
    ...
    <v-row dense v-else>
      <v-col cols="12">
        <v-sheet color="grey lighten-3" class="pa-3">
          <v-skeleton-loader class="mx-auto" width="100%" height="300px" type="card"></v-skeleton-loader>
        </v-sheet>
      </v-col>
    </v-row>
