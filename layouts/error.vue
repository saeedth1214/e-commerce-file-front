    ...
    <v-container fluid>
      <v-row class="pa-4">
        <v-col cols="12" offset-md="3" md="6">
          ...
          <h2 class="status-text">{{ message }}</h2>
          <p class="home">
            <v-btn
              nuxt
              to="/"
...
import {
  ...
  defaultGeneralErrorMessage,
} from "@/plugins/errorMessages";
export default {
  layout: "empty",
  props: {
    error: {
  ...
  async created() {
    if (!defaultErrorMessages[this.error.statusCode]) {
      this.message = await defaultGeneralErrorMessage;
    } else {
      this.message = await defaultErrorMessages[this.error.statusCode];
    }
  },
};
</script>
